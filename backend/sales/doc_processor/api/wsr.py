...
from backend.sales.doc_processor.services.wsr import process_document, get_account_document
...
@router.post("/import_wsr/{account_id}", response_model=ImportResponse)
...
@router.get("/wsr/{account_id}", response_model=None)
async def get_wsr_document(
    ...
    try:
        ...
        if not doc:
            raise HTTPException(status_code=404, detail=f"No WSR document found for account {account_id}")
...
@router.delete("/wsr/{account_id}", response_model=None)
