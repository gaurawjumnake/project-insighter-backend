...
from backend.sales.doc_processor.services.tech_review import process_tech_review_document, get_account_document
...
@router.post("/import_tech_review/{account_id}", response_model=ImportResponse)
...
@router.get("/tech_review/{account_id}", response_model=None)
async def get_tech_review_document(
    ...
    try:
        ...
        if not doc:
            raise HTTPException(status_code=404, detail=f"No Tech Review document found for account {account_id}")
...
@router.delete("/tech_review/{account_id}", response_model=None)
