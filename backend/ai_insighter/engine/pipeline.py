    ...
    def _analysis_task(self, research_task: Task) -> Task:
        return Task(
            description=(
                ...
                "- Generate insights strictly from provided data; do not infer or speculate\n"
                "- Every insight must be traceable to explicit input evidence\n"
                "- Avoid redundant observations and ambiguous wording\n"
                "- Detect and explicitly call out contradictions\n"
                "- If sources conflict, acknowledge inconsistency and explain limitation\n"
                "- If data is missing, state the limitation — do not invent\n"
                "- If a requested area lacks sufficient evidence, state exactly: "
                "'Insufficient data available to generate a reliable insight for this area.'\n"
                "- Avoid unsupported causal claims unless explicit evidence exists\n"
    ...
    def _summarization_task(self, output_format: dict, analysis_task: Task) -> Task:
        ...
        return Task(
            description=(
                ...
                "- Do not fabricate missing values; use explicit insufficiency statements when required\n"
                "- Ensure output is non-redundant, non-conflicting, and evidence-grounded\n"
