...
from backend.doc_insighter.services.best_practices import process_best_practices_document, get_account_document
...
# CHANGED: project_id -> account_id
@router.post("/import_best_practices/{account_id}", response_model=ImportResponse)
async def import_best_practices_document(
    ...
    """
    Import Best Practices document for an account.
    """
    ...
    try:
        # CHANGED: Use account_id for directory creation
        ...
        result = await import_and_save_document(
            file=file,
            account_id=account_id, # CHANGED: Passed as account_id
            temp_dir=TEMP_DIR,
...
# CHANGED: project_id -> account_id
@router.get("/best_practices/{account_id}", response_model=None)
async def get_best_practices_document(
    account_id: UUID,
    db: Session = Depends(get_db)
):
    """
    Retrieve Best Practices document for an account.
    """
    document_type = "best_practices"
    try:
        ...
        if not doc:
            raise HTTPException(status_code=404, detail=f"No Best Practices document found for account {account_id}")
        
        return {
            "document_id": str(doc.id),
            "account_id": str(doc.account_id), # CHANGED
            "content": doc.content,
...
# CHANGED: project_id -> account_id
@router.delete("/best_practices/{account_id}", response_model=None)
async def delete_best_practices_document(
    account_id: UUID,
    db: Session = Depends(get_db)
):
    """
    Delete Best Practices document for an account.
    """
    document_type = "best_practices"
    try:
        doc = get_account_document(db, account_id, document_type) # type:ignore
        ...
        log.log_info(f"Best Practices document deleted for account {account_id}")
        
        return {
            "message": "Best Practices document deleted successfully",
            "document_id": str(doc.id),
            "account_id": str(account_id) # CHANGED
        }
