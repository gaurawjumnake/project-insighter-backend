...
# CHANGED: Import AccountDocument
...
doc_processor = ProcessAccountDocument(BestPractices.prompt,doc_name="Best Practices Document")
...
def get_account_document(
    ...
    """Retrieve document by account and type"""
    return (
        db.query(AccountDocument)
        .options(joinedload(AccountDocument.account)) # CHANGED: joinedload to account
        .filter(
            AccountDocument.account_id == account_id, # CHANGED: account_id
...
def process_best_practices_document(
    db: Session,
    file_path: Path,
    account_id: UUID, # CHANGED: account_id
    dry_run: bool = False
) -> dict[str, Any]:
    ...
    try:
        ...
        if existing_doc:
            ...
            doc_data = AccountDocument( # CHANGED: AccountDocument
                id=uuid4(),
                account_id=account_id,  # CHANGED: account_id
                content=content,
